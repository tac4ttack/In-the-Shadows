...
public class AutoSave : EditorWindow
{
    ...
    private Texture2D AsAsset
    {
        get { return _asAsset ?? (_asAsset = AssetDatabase.LoadAssetAtPath(_path + "asset.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsDisable
    {
        get { return _asDisable ?? (_asDisable = AssetDatabase.LoadAssetAtPath(_path + "disable.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsEnable
    {
        get { return _asEnable ?? (_asEnable = AssetDatabase.LoadAssetAtPath(_path + "enable.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsInfo
    {
        get { return _asInfo ?? (_asInfo = AssetDatabase.LoadAssetAtPath(_path + "info.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsOnOff
    {
        get { return _asOnoff ?? (_asOnoff = AssetDatabase.LoadAssetAtPath(_path + "onoff.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsOnPlay
    {
        get { return _asOnplay ?? (_asOnplay = AssetDatabase.LoadAssetAtPath(_path + "play.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsTime
    {
        get { return _asTime ?? (_asTime = AssetDatabase.LoadAssetAtPath(_path + "time.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D AsBackup
    {
        get { return _asBackup ?? (_asBackup = AssetDatabase.LoadAssetAtPath(_path + "backup.png", typeof(Texture2D)) as Texture2D); }
    }

    private Texture2D PmLogo
    {
        get { return _pmLogo ?? (_pmLogo = AssetDatabase.LoadAssetAtPath(_path + "logo.png", typeof(Texture2D)) as Texture2D); }
    }
    ...
    private void OnGUI()
    {
        ...
        _guiStyleLable.fontSize = 16;
        _guiStyleLable.fontStyle = FontStyle.Bold;
        ...
        if (_saveInterval != EditorPrefs.GetInt("PM_AS_SAVEINTERVAL")
            || _autoSave != EditorPrefs.GetBool("PM_AS_AUTOSAVE")
            || _saveOnPlay != EditorPrefs.GetBool("PM_AS_SAVEONPLAY")
            || _saveAssets != EditorPrefs.GetBool("PM_AS_SAVEASSET")
            || _debugLog != EditorPrefs.GetBool("PM_AS_DEBUGLOG")
            || _backup != EditorPrefs.GetBool("PM_AS_BACKUP")
            || _backupPath != EditorPrefs.GetString("PM_AS_BACKUPPATH"))
        {
            LoadSettings();
        }
